...
def manage_smtp_connection(conn: socket.socket):
    ...
    while True:
        ...
        if token_type == "Activation":
            ...
            with open("/mailserver/mails/mails.txt", "a+") as out_file:
                out_file.write(f"Unknown token type {token_type} in message\n{msg}\n\n")
...
def activation_token_request_service():
    ...
    while True:
        conn, address = soc.accept()
        threading.Thread(target=manage_activation_token_request_connection, args=(conn,)).start()
...
def forgot_token_request_service():
    ...
    while True:
        conn, address = soc.accept()
        threading.Thread(target=manage_forgot_token_request_connection, args=(conn,)).start()
